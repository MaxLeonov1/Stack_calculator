...
Proc_Err_t AsmblrScanFile ( Cmd_Assemblr_t* assmblr ) {
    ...
    line_info.line = strtok ( assmblr->fread_buffer, "\n" );
    line_info.cmd = (char*) calloc ( max_cmd_size, sizeof(char) );
    line_info.arg = (char*) calloc ( max_cmd_size, sizeof(char) );
    
    while (line_info.line != NULL && line_info.cmd_line_num < assmblr->file_lines_num) {
        
        if (line_info.line[0] != '\0') { //TODO Переписать с поэтапным scanf %s
            
            line_info.elements += sscanf ( (const char*)line_info.line, "%s", line_info.cmd ); 
        ...
        line_info.elements = 0;
        line_info.line = strtok(NULL, "\n");
    }
